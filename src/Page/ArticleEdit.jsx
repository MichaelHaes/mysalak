...
const ArticleCms = () => {
  ...
  useEffect(() => {
    const articleToEdit = ArticleData.find((article) => article.id === parseInt(id));
    if (articleToEdit) {
      setArticle(articleToEdit);
      setFotoPenulis(articleToEdit.imageSrc);
      setGambarBacaan(articleToEdit.imageSrc);
    }
  }, [id]);
    ...
    <Box p={8}>
      <Flex justifyContent="center" alignItems="center" position="relative">
        <Box position="absolute" left={0}>
          <BackButton navigateTo="/cms/article" />
        </Box>
        <Heading size="md">Edit Artikel</Heading>
      </Flex>
      <Stack spacing={4} mt={8}>
        <FormControl id="judul">
          <FormLabel fontWeight="bold">Judul</FormLabel>
          <Input placeholder="Masukkan judul bacaan" type="text" defaultValue={article.title} />
        </FormControl>
        <FormControl id="kategori">
          <FormLabel fontWeight="bold">Kategori</FormLabel>
          <Select placeholder="Pilih kategori bacaan" defaultValue={article.tag}>
        ...
        <FormControl id="penulis">
          ...
          <Input placeholder="Masukkan nama penulis" type="text" defaultValue={article.author} />
        </FormControl>
        ...
        <FormControl id="waktu-terbit">
          ...
          <Input type="date" defaultValue={article.date} />
        </FormControl>
        <FormControl id="durasi-membaca">
          <FormLabel fontWeight="bold">Durasi Membaca</FormLabel>
          <Input placeholder="Masukkan durasi membaca bacaan" type="number" defaultValue={article.readTime} />
        </FormControl>
        <FormControl id="isi-bacaan">
            <FormLabel fontWeight="bold">Isi Bacaan</FormLabel>
            <Textarea placeholder="Isi bacaan" height="200px" defaultValue={article.description} />
        </FormControl>
