      ...
      <FormControl isRequired>
        <Flex flexDirection={'column'}>
          {Object.entries(inputs).map(([inputName, value], index) => (
            <Flex key={index}>
              <FormLabel w='100vw' margin={'auto'}>{inputName}</FormLabel>
              <Spacer />
              <Input
                value={value}
                variant='flushed'
          ...
          <Button marginX={'auto'} width={'50%'} marginY={'10px'} onClick={handlePredict}>Prediksi</Button>
        </Flex>
      </FormControl>
      ...
      <Card align='center' w={'90vw'} margin={'auto'}>
        ...
        <CardBody>
          <Flex>
            ...
            <Text>
              {predictions.Temperatur !== 0 ? `: ${parseFloat(predictions.Temperatur).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='40vw'>Kelembapan</Text>
            <Text>
              {predictions.Kelembapan !== 0 ? `: ${parseFloat(predictions.Kelembapan).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='40vw'>Curah Hujan</Text>
            <Text>
              {predictions['Curah Hujan'] !== 0 ? `: ${parseFloat(predictions['Curah Hujan']).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='40vw'>Intensitas Cahaya</Text>
            <Text>
              {predictions['Intensitas Cahaya'] !== 0 ? `: ${parseFloat(predictions['Intensitas Cahaya']).toFixed(2)}` : ':'}
            </Text>
          </Flex>
        </CardBody>
      </Card>
