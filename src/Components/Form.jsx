    ...
    <Box marginX={'10px'} padding={'10px'}>
      ...
      <FormControl>
        {Object.entries(inputs).map(([inputName, value], index) => (
          <Flex>
            <FormLabel w='15vw' margin={'auto'}>{inputName}</FormLabel>
            <Spacer />
            <Input
              key={index}
              value={value}
        ...
        <Button margin={'auto'} onClick={handlePredict}>Prediksi</Button>
      </FormControl>
      ...
      <Card align='center' w={'75%'} margin={'auto'}>
        ...
        <CardBody>
          <Flex>
            ...
            <Text>
              {predictions.Temperatur !== null ? `: ${parseFloat(predictions.Temperatur).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='15vw'>Kelembapan</Text>
            <Text>
              {predictions.Kelembapan !== null ? `: ${parseFloat(predictions.Kelembapan).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='15vw'>Curah Hujan</Text>
            <Text>
              {predictions['Curah Hujan'] !== null ? `: ${parseFloat(predictions['Curah Hujan']).toFixed(2)}` : ':'}
            </Text>
          </Flex>
          <Flex>
            <Text w='15vw'>Intensitas Cahaya</Text>
            <Text>
              {predictions['Intensitas Cahaya'] !== null ? `: ${parseFloat(predictions['Intensitas Cahaya']).toFixed(2)}` : ':'}
            </Text>
          </Flex>
        </CardBody>
      </Card>
